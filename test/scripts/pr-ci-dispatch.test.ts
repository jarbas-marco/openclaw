...
const runUrl = `https://github.com/${repo}/actions/runs/99`;
...
gh() {
  case "$1 $2" in
    ...
    "pr view") printf '{"baseRefOid":"${baseSha}","headRefName":"contributor/fix-hosted-gates","headRefOid":"${headSha}","isCrossRepository":false}\\n' ;;
    *) return 2 ;;
  esac
}
node() { printf '%s\\n' "$*"; }
ci_dispatch 12345`,
    ...
    expect(result.stdout.trim()).toBe(
      `${gatesScriptDirectory}/pr-lib/ci-dispatch.mjs 12345 contributor/fix-hosted-gates ${headSha} ${baseSha} false ${repo}`,
    );
  });
  ...
  it("dispatches ordinary CI for the exact remote head", () => {
    ...
    expect(calls).toContain(
      `real-gh\tworkflow run ci.yml --repo ${repo} --ref contributor/fix-hosted-gates -f target_ref=${headSha} -f release_gate=true -f pull_request_number=12345`,
    );
    expect(calls).toContain(`gh\tpr view 12345 --repo ${repo}`);
  });
  ...
  it("finds the publisher-owned Crabbox proof on a later check-run page", () => {
    ...
    const calls = readFileSync(fakeGh.calls, "utf8");
    expect(calls).toContain(
      `real-gh\tworkflow run pr-crabbox-gate-publisher.yml --repo ${repo} --ref main -f pr_number=12345 -f head_sha=${headSha} -f base_sha=${baseSha}`,
    );
    expect(calls).toContain(`gh\tapi --method GET repos/${repo}/actions/runs/99`);
    expect(calls).toContain(
      `gh\tapi --paginate --slurp repos/${repo}/commits/${headSha}/check-runs?filter=latest&per_page=100`,
    );
  });
  ...
  it("rejects fork PRs before invoking GitHub", () => {
    ...
    const result = spawnSync(
      process.execPath,
      [dispatchScript, "12345", "fix", headSha, baseSha, "true", repo],
      {
        encoding: "utf8",
        env: {
          ...process.env,
  ...
  it("rejects an invalid repository before invoking GitHub", () => {
    ...
    const result = spawnSync(
      process.execPath,
      [dispatchScript, "12345", "fix", headSha, baseSha, "false", "../.."],
      {
        encoding: "utf8",
        env: {
          ...process.env,
