...
function requirePrRecord({ baseRefOid, pr, headRefName, headRefOid, isCrossRepository, repo }) {
...
function listCiRuns(repo, headRefOid, backend) {
  const args =
    backend.name === "crabbox"
      ? [
          ...
          "--method",
          "GET",
          `repos/${repo}/actions/workflows/pr-crabbox-gate-publisher.yml/runs`,
          "-f",
      ...
      : workflowRunsApiArgs(repo, headRefOid, "workflow_dispatch", 20);
...
function readWorkflowRun(repo, runId) {
  return execGhJson(["api", "--method", "GET", `repos/${repo}/actions/runs/${runId}`], {
    stdio: ["ignore", "pipe", "pipe"],
  });
}

function readExactHeadChecks(repo, headSha) {
  const endpoint = `repos/${repo}/commits/${headSha}/check-runs?filter=latest&per_page=100`;
...
function requireUnchangedHead(record, phase, readHeadOid) {
  const current = readHeadOid(record.repo, record.pr);
...
async function waitForCrabboxResult(
  ...
  for (let attempt = 1; attempt <= terminalPollAttempts; attempt += 1) {
    run = readRun(record.repo, observedRun.id);
  ...
  const check = readChecks(record.repo, record.headRefOid)
    .filter(
      (candidate) =>
...
async function dispatchCiForPr(
  ...
  const priorRunIds = new Set(
    listRuns(record.repo, record.headRefOid, backend).map((run) => run.id),
  );
  requireUnchangedHead(record, "before CI dispatch", readHeadOid);
  runDispatch(buildCiDispatchArgs(record, backend));

  for (let attempt = 1; attempt <= pollAttempts; attempt += 1) {
    const run = listRuns(record.repo, record.headRefOid, backend).find((candidate) => {
...
async function main(argv = process.argv.slice(2)) {
  if (argv.length < 6 || !["true", "false"].includes(argv[4])) {
    console.error(
      "Usage: ci-dispatch.mjs <PR> <headRefName> <headRefOid> <baseRefOid> <isCrossRepository> <owner/repository> [--backend crabbox]",
    );
    process.exitCode = 2;
    return;
  }
  ...
  const backend = parseBackendArgs(argv.slice(6));
  requirePrRecord(record);
