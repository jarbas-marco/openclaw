...
ci_dispatch() {
  local pr="$1"
  shift
  local record base_sha head_ref head_sha is_cross_repository repo
  repo=$(gh repo view --json nameWithOwner --jq .nameWithOwner)
  record=$(gh pr view "$pr" --repo "$repo" --json baseRefOid,headRefName,headRefOid,isCrossRepository)
  ...
  node "$script_parent_dir/pr-lib/ci-dispatch.mjs" \
    "$pr" "$head_ref" "$head_sha" "$base_sha" false "$repo" "$@"
}
