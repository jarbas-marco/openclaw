...
function portableCasefoldPath(value: string): string {
  return path
    .resolve(value)
    .replaceAll("\\", "/")
    .replace(/\/+$/u, "")
...
function isPortableCasefoldPathWithin(child: string, parent: string): boolean {
  const normalizedChild = portableCasefoldPath(child);
  const normalizedParent = portableCasefoldPath(parent);
...
async function assertRecoveryProfilePreservedPaths(params: {
  ...
  for (const excludedRoot of BACKUP_RECOVERY_PROFILE_STATE_ROOTS) {
    const excludedPath = path.join(params.stateAsset.sourcePath, excludedRoot);
    for (const protectedPath of params.protectedPaths) {
      const canonicalPath = await canonicalizePathForContainment(protectedPath);
      if (isPortableCasefoldPathWithin(canonicalPath, excludedPath)) {
        throw new Error(
          `Recovery-profile backup cannot preserve excluded state path: ${protectedPath}`,
        );
      }
    }
  }
...
export async function createBackupArchive(
  ...
  if (recoveryProfile) {
    await assertRecoveryProfilePreservedPaths({
      stateAsset: resolveRecoveryProfileStateAsset(plan.included),
      protectedPaths: [
        plan.configPath,
        plan.oauthDir,
        ...plan.workspaceDirs,
        ...plan.inventory.agentRoots.map((root) => root.sourcePath),
      ],
    });
  }
  ...
  try {
    ...
    if (recoveryProfile) {
      try {
        // The omission is part of the restore contract, so verify the staged
        // archive before its final name becomes visible.
