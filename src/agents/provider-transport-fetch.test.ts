...
describe("buildGuardedModelFetch", () => {
  ...
  it("allows text/plain when streamed OpenAI-compatible responses contain SSE", async () => {
    fetchWithSsrFGuardMock.mockResolvedValue({
      response: new Response(responseStreamText('data: {"ok": true}\n\ndata: [DONE]\n\n'), {
  ...
  it("synthesizes SSE for text/plain JSON returned to streaming SDK requests", async () => {
    fetchWithSsrFGuardMock.mockResolvedValue({
  ...
  it("rejects text/plain HTML returned to streaming SDK requests", async () => {
    const release = vi.fn(async () => undefined);
    fetchWithSsrFGuardMock.mockResolvedValue({
