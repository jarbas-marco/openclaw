...
export type BackupRegenerableKind =
  ...
  | typeof BACKUP_RECOVERY_PROFILE_SKIP_KIND
  | "managed state"
  | "plugin skills"
  | "plugin resource"
  | "plugin dependencies";
...
type BackupRegenerableRoot = Readonly<{
  ...
  sourcePath: string;
  reason?: typeof BACKUP_RECOVERY_PROFILE_SKIP_REASON;
  strict?: boolean;
  portableCasefold?: boolean;
}>;
...
export async function createBackupResourceInventory(params: {
  ...
  const exclude = (
    kind: BackupRegenerableKind,
    sourcePath: string,
    options: Pick<BackupRegenerableRoot, "portableCasefold" | "reason" | "strict"> = {},
  ): void => {
    regenerableRoots.push({ kind, sourcePath: path.resolve(sourcePath), ...options });
  };
  ...
  if (!params.onlyConfig) {
    ...
    if (params.recoveryProfile) {
      for (const root of BACKUP_RECOVERY_PROFILE_STATE_ROOTS) {
        exclude(BACKUP_RECOVERY_PROFILE_SKIP_KIND, path.join(stateDir, root), {
          portableCasefold: true,
  ...
  const portableCasefoldPath = (value: string): string =>
  ...
  const isWithinExcludedResource = (
    ...
    const child = portableCasefoldPath(candidate);
    const parent = portableCasefoldPath(resource.sourcePath);
    return child === parent || child.startsWith(`${parent}/`);
  };

  const isIncluded = (sourcePath: string): boolean => {
    const candidate = path.resolve(sourcePath);
    const exclusion = excludedResources.find((resource) =>
      isWithinExcludedResource(candidate, resource),
    );
    if (!exclusion) {
      return true;
    }
    ...
    return protectedPaths.some(
      (protectedPath) =>
        isPathWithin(candidate, protectedPath) && isPathWithin(protectedPath, exclusion.sourcePath),
    );
  };
