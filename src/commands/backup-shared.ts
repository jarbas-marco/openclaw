...
import { BACKUP_RECOVERY_PROFILE_SKIP_REASON } from "../infra/backup-recovery-profile.js";
...
type BackupSkipReason =
  ...
  | "regenerable"
  | "unresolved"
  | typeof BACKUP_RECOVERY_PROFILE_SKIP_REASON;
...
async function resolveBackupPlanFromPaths(params: {
  ...
  for (const [index, resource] of regenerableRoots.entries()) {
    if (!regenerableResourceExists[index] && !resource.reason) {
      continue;
    }
    skipped.push({
      kind: resource.kind,
      ...
      reason: resource.reason ?? "regenerable",
    });
  }
